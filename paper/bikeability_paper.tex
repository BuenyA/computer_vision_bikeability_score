...
% =====================================================================
% PAGE 3 -- OBJECT DETECTION (Gonzalo-- heading only)
% =====================================================================
...
For the object recognition module, the methodology focuses specifically
on counting objects.
This design choice was made because objects move relative to the bike
rider regardless of their absolute speed, as the rider is in continuous
motion.
Consequently, an object's size within the field of view changes frame
by frame and cannot serve as a stable factor for calculations.

This dynamic property led to another design consideration regarding the
scope of the \enquote{bikeability score} evaluator: evaluating the
exact same route will yield slightly different results depending on
the specific time the footage is captured.
For this algorithm to be viable in a mass-market product, it must be
capable of running in real time on an edge device mounted on the
bicycle.
This strict real-time requirement applies to the dynamic object
detection, but not to the other two static sub-scores.

\subsection{Performance Trade-offs}
Therefore, the primary selection parameters for this specific task are
the model's inference time and its overall size, which are inherently
related.
Because larger models can extract more complex image features, there
is a direct trade-off between inference time and accuracy, requiring
us to find an optimal balance for our model selection.

These runtime criteria, combined with the fact that this module only
requires object recognition, led directly to targeting the YOLO family
of models as the most suitable candidates for the task.
However, it remains necessary to benchmark different generations and
sizes of YOLO models to determine the most effective configuration.
...
custom retraining.
Consequently, the COCO dataset convention was selected as the primary
starting point, though Mapillary remains a viable alternative dataset
for future iterations.

The COCO dataset classifies detected objects into 80 distinct
categories, not all of which are relevant to traffic environments.
Therefore, the relevant classes were grouped into three primary
subsets (Table~\ref{tab:coco_subsets}).

\begin{table}[H]
\caption{Grouping of relevant COCO object classes into traffic-related
categories.}
\label{tab:coco_subsets}
\centering
\small
\begin{tabular}{@{}lp{4.8cm}@{}}
\toprule
\textbf{Category} & \textbf{COCO Object Classes}\\
\midrule
Motor vehicles & car, truck, bus, motorcycle\\
Other road users & person, bicycle\\
...
From these subsets, three representative classes were selected:
\textbf{car}, \textbf{bicycle}, and \textbf{traffic light}.

To ensure accurate labeling within the reference dataset, it is
necessary to strictly define the boundaries and potential edge cases
for the target classes (Table~\ref{tab:object_classes}).
