...
score. Frames come from a handlebar-mounted camera, sampled at one
frame every five seconds; as each frame is dominated by one surface,
the task is single-label image classification, not segmentation.
...
bounded to $[0, 100]$; in practice the extremes $0$ and $100$ are never
reached, as $S_{\text{ground}}\ge 0.1$ and
$S_{\text{object}}=\exp(-\max(\rho,0))>0$.
