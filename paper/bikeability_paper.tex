...
\clearpage
\section{Ground Detection: Road-Surface Classification}

The ridden surface is a primary determinant of cycling comfort and
safety: a paved cycleway can be ridden effortlessly, whereas loose
gravel or bare soil is strenuous and, on a road bike, potentially
hazardous. Ground detection therefore assigns each sampled frame a
single surface class and supplies the surface term of the bikeability
score. Frames come from a handlebar-mounted camera at one frame per
second; as each frame is dominated by one surface, the task is
single-label image classification, not segmentation.

\subsection{Task Definition and Model Selection}
Two model families can solve this task without an existing labeled
corpus: a classifier \emph{trained on our own labels}, or a
\emph{zero-shot} vision-language model (VLM). We avoid a self-trained
classifier, as it would need a large, balanced, manually labeled
training set, careful regularization on limited data, and complete
re-training whenever the class definitions change. A zero-shot VLM of
the CLIP family \cite{clip} instead classifies an image from its
similarity to natural-language class descriptions, so the taxonomy can
be adapted by editing text -- without task-specific training.

One distinction is often blurred: \emph{linear probing} of
self-supervised features (e.g., DINOv2 \cite{dinov2}) is \emph{not}
zero-shot -- its linear head is fitted on labeled target data and is
therefore supervised; it is excluded here and would only qualify as a
declared supervised baseline.
...
prompts whose normalized text embeddings are averaged (\emph{prompt
ensembling}). Since Cycleway and Road share the same smooth-asphalt
texture, the prompts encode the \emph{discriminating context} -- a
narrow separated lane with bicycle markings versus a wide multi-lane
road -- to push the two embeddings apart. A frame is assigned to the
class of highest cosine similarity via a softmax.
...
from self-recorded rides; frames without a clear surface were tagged
\emph{unclear/other} and excluded. As the classes are imbalanced, we
report accuracy, balanced accuracy and macro-F1 with $95\%$ bootstrap
confidence intervals and McNemar tests; latency is measured at batch
size~1 with warm-up runs (ms/frame and FPS).
...
Consistently, OpenCLIP ViT-B/32 (LAION-2B) also attains the clearly
highest balanced accuracy ($0.72$ vs.\ $\le\!0.70$ for every other
model). Second, the $95\%$ bootstrap accuracy intervals of the leading
...
despite being the largest model it is both slowest and weakest ($0.42$
macro-F1), confirming that capacity alone does not help. As
Fig.~\ref{fig:confusion} shows, the residual errors concentrate on the
Cycleway/Road pair (both smooth asphalt) and on the rare Gravel and
Unpaved classes, which hold only $51$ and $13$ labeled frames and thus
cap the macro-F1 at $0.58$ -- still sufficient for one of three
sub-scores. We therefore adopt OpenCLIP ViT-B/32 (LAION-2B) as the
ground detector. As all models share the same input and
\texttt{open\_clip} pipeline, the differences reflect architecture and
training data; the CPU latencies are hardware-dependent.
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.74\columnwidth]{images/fig_confusion_best.pdf}
...
\qquad
\rho = \frac{1}{N}\,\langle \mathbf{n},\, \boldsymbol{\lambda}\rangle,
\end{equation}
with the number of aggregated frames $N$ (here $N=1$ per frame). A
positive penalty for cars ($+1.0$) and traffic lights ($+0.3$) lowers
the sub-score, while bicycle-friendly objects such as other cyclists
carry a \emph{negative} factor ($-0.2$) and raise it. The exponential
form is robust against outliers, so a single crowded frame cannot
...
penalty $\boldsymbol{\lambda}$ (heuristic).}
\label{tab:quality}
\centering
\begin{tabular}{@{}llc@{}}
\toprule
...
\multirow{3}{*}{Environment $\mathbf{q}^{e}$}
  ...
  & City       & $0.4$\\
\midrule
\multirow{3}{*}{Object penalty $\boldsymbol{\lambda}$}
  & Bicycle       & $-0.2$\\
  & Car           & $+1.0$\\
  & Traffic light & $+0.3$\\
\bottomrule
...
\subsection{Route Processing over the Full Ride}
The action camera splits one continuous recording into several files
(e.g., \texttt{DJI\_0408}, \texttt{DJI\_0409}, \dots). All parts are
processed as a \emph{single} continuous video: the files are ordered
chronologically by the number in their name, and a cumulative time
offset (the sum of the previous parts' durations) keeps the timeline
gap-free across file boundaries. Within each part, one frame is sampled
every $5$\,s by \emph{seeking} directly to the target frame index
instead of decoding every frame, which removes almost all of the
decoding work. The three detectors are loaded once and evaluated in
batches; for every sampled frame, the three output vectors and the
resulting score are written to a shared CSV (frame name, continuous
time in seconds and \texttt{HH:MM:SS}, an optional absolute ISO
timestamp, the serialized vectors, and the score).
...
tolerance), which assigns a latitude/longitude to every score. The
\emph{average} bikeability score of the route is the mean over all $M$
evaluated frames,
\begin{equation}
\label{eq:avg}
...
$\bar{A}\approx 85.5/100$: the track is predominantly green (good
