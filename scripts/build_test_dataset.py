...
Collects labeling candidates for the 4 environment classes
(forest, open_field, water, city) from three sources:
...
# scarce classes (water, open_field) are listed first to get first pick of
# each image. Thresholds are deliberately loose: Vistas is urban street-level
# footage where water/fields are rare, and the human labeler is the ground
# truth and rejects false positives, so we optimise for candidate recall here.
MAPILLARY_RULES = {
    "water": lambda s: s["water"] >= 0.01,
    "open_field": lambda s: s["terrain"] >= 0.10 and s["vegetation"] < 0.30
    and s["building"] <= 0.10,
    "forest": lambda s: s["vegetation"] >= 0.25 and s["building"] <= 0.15,
...
def scan_mapillary(root: Path, per_class, seed, max_scan, train_fallback=True):
    ...
    footage, so classes like forest/water can be rare there; unlike ADE20K,
