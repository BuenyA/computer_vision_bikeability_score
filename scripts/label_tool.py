...
Keys:
  f / o / w / c       toggle forest / open_field / water / city
  ...
  x                   clear all toggles for the current image
...
CSV schema (multi-hot, one row per image):
  ...
  forest, open_field, water, city, unsure, reject
...
Usage:
  python scripts/label_tool.py                    # default folders
  python scripts/label_tool.py --images DIR --csv FILE
...
CLASSES = ["forest", "open_field", "water", "city"]
KEY_TO_CLASS = {ord("f"): "forest", ord("o"): "open_field",
                ord("w"): "water", ord("c"): "city"}
CLASS_BGR = {  # given palette (RGB) converted to OpenCV BGR
    "forest": (40, 120, 30),
    "open_field": (100, 220, 180),
    "water": (180, 130, 0),
    "city": (130, 130, 130),
}
...
def order_key(rel: str):
    ...
    forest/water external buckets, then everything else; path-sorted within."""
    parts = rel.replace("\\", "/").split("/")
    top = parts[0]
    if top == "own_frames":
        return (0, rel)
    if len(parts) > 1 and parts[1] in ("forest", "water"):
        return (1, rel)
    return (2, rel)
...
def collect_images(root: Path):
    ...
    return sorted(imgs, key=lambda p: order_key(str(p.relative_to(root))))
...
def render(img, rel, toggles, unsure, reject, idx, total, n_labeled, is_saved):
    ...
    help_line = ("f/o/w/c toggle   Space save+next   Backspace back   "
                 "u unsure   r reject   x clear   s save   q quit")
